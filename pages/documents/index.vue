...
<template>
  <div class="grid grid-cols-4 gap-2 mt-12" v-if="isLoading">
    <USkeleton
      class="h-12 dark:bg-gray-900 bg-gray-300"
      v-for="item in data"
      :key="item.id"
    />
    <UiDealsLoader v-for="item in data" :key="item.id" />
  </div>
  <div class="grid grid-cols-4 gap-2 mt-12 select-none" v-else>
    <div
      ...
      :class="
        isMoving &&
        'border-l-2 border-r-2 border-dotted h-screen border-gray-200 dark:border-gray-900'
      "
    >
      ...
      >
        <div class="flex items-center space-x-2">
          <span class="uppercase font-bold text-lg">{{ deal.name }}</span>
        </div>
        <UDivider class="my-3" />
